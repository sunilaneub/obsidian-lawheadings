...
var DEFAULT_SETTINGS = {
  levels: [
    { numberStyle: "upperAlpha", suffixStyle: "dot" },
    // H1: A. B. C.
    { numberStyle: "roman", suffixStyle: "dot" },
    // H2: I. II. III.
    { numberStyle: "arabic", suffixStyle: "dot" },
    // H3: 1. 2. 3.
    { numberStyle: "lowerAlpha", suffixStyle: "paren" },
    // H4: a) b) c)
    { numberStyle: "doubleLowerAlpha", suffixStyle: "paren" },
    // H5: aa) bb) cc)
    { numberStyle: "arabic", suffixStyle: "parens" }
    // H6: (1) (2) (3)
  ]
};
var NUMBER_STYLE_LABELS = {
...
var SUFFIX_STYLE_LABELS = {
  dot: "Punkt: X.",
  paren: "Klammer: X)",
  parens: "In Klammern: (X)"
};
function toUpperAlpha(n) {
  let result = "";
  while (n > 0) {
    n--;
...
function convertNumber(style, count) {
  switch (style) {
    case "upperAlpha":
      return toUpperAlpha(count);
    case "lowerAlpha":
      return toLowerAlpha(count);
    case "doubleLowerAlpha":
      return toDoubleLowerAlpha(count);
    case "roman":
...
function applySuffix(num, suffix) {
  switch (suffix) {
    case "dot":
      return num + ".";
    case "paren":
...
function formatNumber(level, count, settings) {
  const fmt = settings.levels[level];
  if (!fmt || fmt.numberStyle === "none")
    return "";
  const num = convertNumber(fmt.numberStyle, count);
  return applySuffix(num, fmt.suffixStyle);
}
var STRIP_PATTERN = /^(?:\([A-Za-z]+\)|\([IVXLCDM]+\)|\(\d+\)|[A-Z]+\.|[IVXLCDM]+\.|[a-z]{2}\)|[a-z]\)|\d+\.)\s*/;
function stripExistingNumber(text) {
  return text.replace(STRIP_PATTERN, "");
}
function computeHeadingChanges(doc, settings) {
...
var LawHeadingsPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
...
var LawHeadingsSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    for (let level = 0; level < 6; level++) {
      ...
      const preview = fmt.numberStyle === "none" ? "keine" : applySuffix(convertNumber(fmt.numberStyle, level + 1), fmt.suffixStyle);
      ...
      new import_obsidian.Setting(containerEl).setName("Nummernstil").addDropdown((dropdown) => {
        for (const [key, label] of Object.entries(NUMBER_STYLE_LABELS)) {
          dropdown.addOption(key, label);
        }
        dropdown.setValue(fmt.numberStyle);
        dropdown.onChange(async (value) => {
          this.plugin.settings.levels[level].numberStyle = value;
          await this.plugin.saveSettings();
          this.display();
        });
      });
      if (fmt.numberStyle !== "none") {
        new import_obsidian.Setting(containerEl).setName("Suffix-Format").addDropdown((dropdown) => {
          for (const [key, label] of Object.entries(SUFFIX_STYLE_LABELS)) {
            dropdown.addOption(key, label);
          }
          dropdown.setValue(fmt.suffixStyle);
          dropdown.onChange(async (value) => {
            this.plugin.settings.levels[level].suffixStyle = value;
            await this.plugin.saveSettings();
            this.display();
          });
        });
